...
/** An iterator that fetches multiple blocks. For local blocks, it fetches from the local block manager. For remote
  * blocks, it fetches them using the provided BlockTransferService.
  *
  * This creates an iterator of (BlockID, InputStream) tuples so the caller can handle blocks in a pipelined fashion as
  * they are received.
  *
  * The implementation throttles the remote fetches so they don't exceed maxBytesInFlight to avoid using too much
  * memory.
  *
  * @param context
  ...
  *   list of blocks to fetch grouped by the [[BlockManagerId]]. For each block we also require two info: 1. the size
  *   (in bytes as a long field) in order to throttle the memory usage; 2. the mapIndex for this block, which indicate
  *   the index in the map stage. Note that zero-sized blocks are already excluded, which happened in
  *   [[org.apache.spark.MapOutputTracker.convertMapStatuses]].
  * @param mapOutputTracker
  *   [[MapOutputTracker]] for falling back to fetching the original blocks if we fail to fetch shuffle chunks when push
  *   based shuffle is enabled.
  * @param streamWrapper
  ...
  *   whether the shuffle checksum is enabled. When enabled, Spark will try to diagnose the cause of the block
  *   corruption.
  ...
  */
private[spark] final class S3ShuffleBlockFetcherIterator(
    context: TaskContext,
  ...
  /** The number of blocks processed by the caller. The iterator is exhausted when [[numBlocksProcessed]] ==
    * [[numBlocksToFetch]].
    */
  ...
  /** Current [[FetchResult]] being processed. We track this so we can release the current buffer in case of a runtime
    * exception when processing the current buffer.
    */
  ...
  /** Count the retry times for the blocks due to Netty OOM. The block will stop retry if retry times has exceeded the
    * [[maxAttemptsOnNettyOOM]].
    */
  private[this] val blockOOMRetryCounts = new HashMap[String, Int]

  /** The blocks that can't be decompressed successfully, it is used to guarantee that we retry at most once for those
    * corrupted blocks.
    */
  private[this] val corruptedBlocks = mutable.HashSet[BlockId]()

  /** Whether the iterator is still active. If isZombie is true, the callback interface will no longer place fetched
    * blocks into [[results]].
    */
  @GuardedBy("this")
  private[this] var isZombie = false

  /** A set to store the files used for shuffling remote huge blocks. Files in this set will be deleted when cleanup.
    * This is a layer of defensiveness against disk file leaks.
    */
  @GuardedBy("this")
  private[this] val shuffleFilesSet = mutable.HashSet[DownloadFile]()

  private[this] val onCompleteCallback = new S3ShuffleFetchCompletionListener(this)

  private[this] val pushBasedFetchHelper =
    new S3PushBasedFetchHelper(this, shuffleClient, blockManager, mapOutputTracker, shuffleMetrics)
  private[this] val fallbackStorageExecutorsService =
    ThreadUtils.newDaemonCachedThreadPool("fallback-storage-future", fallbackParallelism)
  initialize()
  ...
  private[this] def sendRequest(req: FetchRequest): Unit = {
    ...
    val infoMap = req.blocks.map { case FetchBlockInfo(blockId, size, mapIndex) =>
      (blockId.toString, (size, mapIndex))
    }.toMap
    ...
    val blockFetchingListener = new BlockFetchingListener {
      override def onBlockFetchSuccess(blockId: String, buf: ManagedBuffer): Unit = {
        ...
        S3ShuffleBlockFetcherIterator.this.synchronized {
          if (!isZombie) {
      ...
      override def onBlockFetchFailure(blockId: String, e: Throwable): Unit = {
        S3ShuffleBlockFetcherIterator.this.synchronized {
          ...
          e match {
            ...
            case _: OutOfDirectMemoryError if blockOOMRetryCounts.getOrElseUpdate(blockId, 0) < maxAttemptsOnNettyOOM =>
              if (!isZombie) {
            ...
            case _ =>
              ...
              if (block.isShuffleChunk) {
                ...
                results.put(
                  FallbackOnPushMergedFailureResult(block, address, infoMap(blockId)._1, remainingBlocks.isEmpty)
                )
              } else {
  ...
  /** This is called from initialize and also from the fallback which is triggered from [[PushBasedFetchHelper]].
    */
  private[this] def partitionBlocksByFetchMode(
    ...
    assert(
      blocksToFetchCurrentIteration == localBlocks.size + fallbackBlocks.size +
      ...
      s"The number of non-empty blocks $blocksToFetchCurrentIteration doesn't equal to the sum " +
        ...
        s"of the number of fallback blocks ${fallbackBlocks.size} + " +
    ...
    logInfo(
      s"Getting $blocksToFetchCurrentIteration " +
        ...
        s"${localBlocks.size + fallbackBlocks.size} (${Utils.bytesToString(localBlockBytes)})" +
      ...
      blockId match {
        ...
        case ShuffleBlockChunkId(_, _, _, _) =>
          if (
            ...
            curBlocks =
              createFetchRequests(curBlocks, address, isLast = false, collectedRemoteRequests, enableBatchFetch = false)
        ...
        case _ =>
          ...
          if (curRequestSize >= targetRemoteRequestSize || mayExceedsMaxBlocks) {
            curBlocks = createFetchRequests(curBlocks, address, isLast = false, collectedRemoteRequests, doBatchFetch)
            curRequestSize = curBlocks.map(_.size).sum
          }
      }
    }
  ...
  /** Fetch the local blocks while we are fetching remote blocks. This is ok because `ManagedBuffer`'s memory is
    * allocated lazily when we create the input stream, so all we track in-memory are the ManagedBuffer references
    * themselves.
    */
  ...
  private[this] def fetchHostLocalBlock(
    ...
    try {
      ...
      buf.retain()
      results.put(SuccessFetchResult(blockId, mapIndex, blockManagerId, buf.size(), buf, isNetworkReqDone = false))
      true
    } catch {
      case e: Exception =>
        // If we see an exception, stop immediately.
  ...
  /** Fetch the host-local blocks while we are fetching remote blocks. This is ok because `ManagedBuffer`'s memory is
    * allocated lazily when we create the input stream, so all we track in-memory are the ManagedBuffer references
    * themselves.
    */
      ...
      .map { case (blockId, mapIndex) =>
        val (shuffleId, mapId) = blockId match {
          ...
          case batchId: ShuffleBlockBatchId =>
            (batchId.shuffleId, batchId.mapId)
          case _ =>
  ...
  /** Fetches the next (BlockId, InputStream). If a task fails, the ManagedBuffers underlying each InputStream will be
    * freed by the cleanup() method registered with the TaskCompletionListener. However, callers should close() these
    * InputStreams as soon as they are no longer needed, in order to release memory as early as possible.
    *
  ...
  override def next(): (BlockId, InputStream) = {
    ...
    while (result == null) {
      ...
      result match {
              ...
              case e: IOException =>
                ...
                if (blockId.isShuffleChunk) {
                  ...
                  if (checksumEnabled) {
                    ...
                    throwFetchFailedException(blockId, mapIndex, address, e, Some(diagnosisResponse))
                  } else {
                    throwFetchFailedException(blockId, mapIndex, address, e)
                  }
                } else {
        ...
        case PushMergedLocalMetaFetchResult(shuffleId, shuffleMergeId, reduceId, bitmaps, localDirs) =>
          ...
          try {
            ...
            bufs.zipWithIndex.foreach { case (buf, chunkId) =>
              buf.retain()
              val shuffleChunkId = ShuffleBlockChunkId(shuffleId, shuffleMergeId, reduceId, chunkId)
    ...
    (
      ...
      new S3BufferReleasingInputStream(
        input,
        this,
        currentResult.blockId,
  ...
  /** Get the suspect corruption cause for the corrupted block. It should be only invoked when checksum is enabled and
    * corruption was detected at least once.
    *
    * This will firstly consume the rest of stream of the corrupted block to calculate the checksum of the block. Then,
    * it will raise a synchronized RPC call along with the checksum to ask the server(where the corrupted block is
    * fetched from) to diagnose the cause of corruption and return it.
    *
    * Any exception raised during the process will result in the [[Cause.UNKNOWN_ISSUE]] of the corruption cause since
    * corruption diagnosis is only a best effort.
    *
    * @param checkedIn
  ...
  private[storage] def diagnoseCorruption(
    ...
    blockId match {
      ...
      case unexpected: BlockId =>
        throw SparkException.internalError(s"Unexpected type of BlockId, $unexpected", category = "STORAGE")
    }
  }

  def toCompletionIterator: Iterator[(BlockId, InputStream)] = {
    CompletionIterator[(BlockId, InputStream), this.type](this, onCompleteCallback.onComplete(context))
  }
  ...
  private[storage] def throwFetchFailedException(
    ...
    blockId match {
      ...
      case ShuffleBlockBatchId(shuffleId, mapId, startReduceId, _) =>
        throw SparkCoreErrors.fetchFailedError(address, shuffleId, mapId, mapIndex, startReduceId, msg, e)
  ...
  /** Currently used by [[PushBasedFetchHelper]] to fetch fallback blocks when there is a fetch failure related to a
    * push-merged block or shuffle chunk. This is executed by the task thread when the `iterator.next()` is invoked and
    * if that initiates fallback.
    */
  private[storage] def fallbackFetch(
    ...
    enqueueLocalStorageFetch(originalFallbackBlocks.toSeq)
  ...
  /** Removes all the pending shuffle chunks that are on the same host and have the same reduceId as the current chunk
    * that had a fetch failure. This is executed by the task thread when the `iterator.next()` is invoked and if that
    * initiates fallback.
    *
    * @return
  ...
  private[storage] def removePendingChunks(
      ...
      address: BlockManagerId
  ): mutable.HashSet[ShuffleBlockChunkId] = {
...
/** Helper class that ensures a ManagedBuffer is released upon InputStream.close() and also detects stream corruption if
  * streamCompressedOrEncrypted is true
  */
private class S3BufferReleasingInputStream(
    // This is visible for testing
    private[storage] val delegate: InputStream,
    private val iterator: S3ShuffleBlockFetcherIterator,
  ...
  /** Execute a block of code that returns a value, close this stream quietly and re-throwing IOException as
    * FetchFailedException when detectCorruption is true. This method is only used by the `available`, `read` and `skip`
    * methods inside `BufferReleasingInputStream` currently.
    */
...
/** A listener to be called at the completion of the ShuffleBlockFetcherIterator
  ...
  */
private class S3ShuffleFetchCompletionListener(var data: S3ShuffleBlockFetcherIterator) extends TaskCompletionListener {
...
private[storage] object S3ShuffleBlockFetcherIterator {

  /** A flag which indicates whether the Netty OOM error has raised during shuffle. If true, unless there's no in-flight
    * fetch requests, all the pending shuffle fetch requests will be deferred until the flag is unset (whenever there's
    * a complete fetch request).
    */
  val isNettyOOMOnShuffle = new AtomicBoolean(false)

  val loader: CacheLoader[String, Object] = new CacheLoader[String, Object]() {
    override def load(key: String): Object = {
      return new Object()
    }
  }
  val lockCache: LoadingCache[String, Object] = CacheBuilder.newBuilder.maximumSize(1000).build(loader)
  ...
  /** This function is used to merged blocks when doBatchFetch is true. Blocks which have the same `mapId` can be merged
    * into one block batch. The block batch is specified by a range of reduceId, which implies the continuous shuffle
    * blocks that we can fetch in a batch. For example, input blocks like (shuffle_0_0_0, shuffle_0_0_1, shuffle_0_1_0)
    * can be merged into (shuffle_0_0_0_2, shuffle_0_1_0_1), and input blocks like (shuffle_0_0_0_2, shuffle_0_0_2,
      ...
      def mergeFetchBlockInfo(toBeMerged: ArrayBuffer[FetchBlockInfo]): FetchBlockInfo = {
        ...
        val (startReduceId, size) =
          if (mergedBlockInfo.nonEmpty && shouldMergeIntoPreviousBatchBlockId) {
            ...
            (removed.blockId.asInstanceOf[ShuffleBlockBatchId].startReduceId, removed.size + toBeMerged.map(_.size).sum)
          } else {
  ...
  /** The block information to fetch used in FetchRequest.
    ...
    *   block id
    * @param size
    *   estimated size of the block. Note that this is NOT the exact bytes. Size of remote block is used to calculate
    *   bytesInFlight.
    * @param mapIndex
  ...
  /** A request to fetch blocks from a remote BlockManager.
    ...
    *   true if this request is for requesting push-merged meta information; false if it is for regular or shuffle
    *   chunks.
    */
  ...
  /** Result of a fetch from a remote block unsuccessfully.
    ...
    */
  private[storage] case class FailureFetchResult(blockId: BlockId, mapIndex: Int, address: BlockManagerId, e: Throwable)
      extends FetchResult

  /** Result of a fetch request that should be deferred for some reasons, e.g., Netty OOM
    */
  private[storage] case class DeferFetchRequestResult(fetchRequest: FetchRequest) extends FetchResult

  /** Result of an un-successful fetch of either of these: 1) Remote shuffle chunk. 2) Local push-merged block.
    *
    ...
    * @param isNetworkReqDone
    *   Is this the last network request for this host in this fetch request. Used to update reqsInFlight.
    */
