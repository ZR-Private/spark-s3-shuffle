...
import org.apache.spark.storage.S3ShuffleBlockFetcherIterator._
...
/** Helper class for [[ShuffleBlockFetcherIterator]] that encapsulates all the push-based functionality to fetch
  * push-merged block meta and shuffle chunks. A push-merged block contains multiple shuffle chunks where each shuffle
  * chunk contains multiple shuffle blocks that belong to the common reduce partition and were merged by the external
  * shuffle service to that chunk.
  */
private class S3PushBasedFetchHelper(
  ...
  /** This is executed by the task thread when the `iterator.next()` is invoked and the iterator processes a response of
    * type [[ShuffleBlockFetcherIterator.SuccessFetchResult]].
    *
    * @param blockId
    *   shuffle chunk id.
    */
  ...
  /** This is executed by the task thread when the `iterator.next()` is invoked and the iterator processes a response of
    * type [[ShuffleBlockFetcherIterator.PushMergedLocalMetaFetchResult]].
    *
    * @param blockId
    *   shuffle chunk id.
    */
  ...
  /** This is executed by the task thread when the `iterator.next()` is invoked and the iterator processes a response of
    * type [[ShuffleBlockFetcherIterator.PushMergedRemoteMetaFetchResult]].
    *
    * @param shuffleId
    *   shuffle id.
    * @param reduceId
  ...
  /** This is executed by the task thread when the iterator is initialized and only if it has push-merged blocks for
    * which it needs to fetch the metadata.
    *
    * @param req
    *   [[ShuffleBlockFetcherIterator.FetchRequest]] that only contains requests to fetch metadata of push-merged
    *   blocks.
    */
  def sendFetchMergedStatusRequest(req: FetchRequest): Unit = {
    ...
    val mergedBlocksMetaListener = new MergedBlocksMetaListener {
      override def onSuccess(shuffleId: Int, shuffleMergeId: Int, reduceId: Int, meta: MergedBlockMeta): Unit = {
        logDebug(
        ...
        try {
          ...
          case exception: Exception =>
            logError(
              s"Failed to parse the meta of push-merged block for ($shuffleId, " +
                s"$shuffleMergeId, $reduceId) from" +
                s" ${req.address.host}:${req.address.port}",
              exception
            )
            iterator.addToResultsQueue(
      ...
      override def onFailure(shuffleId: Int, shuffleMergeId: Int, reduceId: Int, exception: Throwable): Unit = {
        logError(
          s"Failed to get the meta of push-merged block for ($shuffleId, $reduceId) " +
            s"from ${req.address.host}:${req.address.port}",
          exception
        )
        iterator.addToResultsQueue(PushMergedRemoteMetaFailedFetchResult(shuffleId, shuffleMergeId, reduceId, address))
      }
    }
  ...
  /** This is executed by the task thread when the iterator is initialized. It fetches all the outstanding push-merged
    * local blocks.
    * @param pushMergedLocalBlocks
    *   set of identified merged local blocks and their sizes.
    */
  def fetchAllPushMergedLocalBlocks(pushMergedLocalBlocks: mutable.LinkedHashSet[BlockId]): Unit = {
  ...
  /** Fetch the push-merged blocks dirs if they are not in the cache and eventually fetch push-merged local blocks.
    */
  private def fetchPushMergedLocalBlocks(
    ...
    val cachedPushedMergedDirs = hostLocalDirManager.getCachedHostLocalDirsFor(SHUFFLE_MERGER_IDENTIFIER)
    if (cachedPushedMergedDirs.isDefined) {
      ...
      pushMergedLocalBlocks.foreach { blockId =>
        fetchPushMergedLocalBlock(blockId, cachedPushedMergedDirs.get, localShuffleMergerBlockMgrId)
      }
    } else {
      ...
      hostLocalDirManager.getHostLocalDirs(
        ...
        case Success(dirs) =>
          ...
          pushMergedLocalBlocks.foreach { blockId =>
            ...
            fetchPushMergedLocalBlock(blockId, dirs(SHUFFLE_MERGER_IDENTIFIER), localShuffleMergerBlockMgrId)
          }
        case Failure(throwable) =>
          ...
          logWarning(
            s"Error while fetching the merged dirs for push-merged-local " +
              s"blocks: ${pushMergedLocalBlocks.mkString(", ")}. Fetch the original blocks instead",
            throwable
          )
          pushMergedLocalBlocks.foreach { blockId =>
            iterator.addToResultsQueue(
              FallbackOnPushMergedFailureResult(blockId, localShuffleMergerBlockMgrId, 0, isNetworkReqDone = false)
            )
          }
      }
    }
  ...
  /** Fetch a single push-merged-local block generated. This can also be executed by the task thread as well as the
    * netty thread.
    * @param blockId
  ...
  private[this] def fetchPushMergedLocalBlock(
    ...
    try {
      ...
      case e: Exception =>
        ...
        logWarning(
          s"Error occurred while fetching push-merged-local meta, " +
  ...
  /** This is executed by the task thread when the `iterator.next()` is invoked and the iterator processes a response of
    * type: 1) [[ShuffleBlockFetcherIterator.SuccessFetchResult]] 2)
    ...
    *
    * This initiates fetching fallback blocks for a push-merged block or a shuffle chunk that failed to fetch. It makes
    * a call to the map output tracker to get the list of original blocks for the given push-merged block/shuffle chunk,
    * split them into remote and local blocks, and process them accordingly. It also updates the numberOfBlocksToFetch
    * in the iterator as it processes failed response and finds more push-merged requests to remote and again updates it
    * with additional requests for original blocks. The fallback happens when:
    *   1. There is an exception while creating shuffle chunks from push-merged-local shuffle block. See
    *      fetchLocalBlock. 2. There is a failure when fetching remote shuffle chunks. 3. There is a failure when
    *      processing SuccessFetchResult which is for a shuffle chunk (local or remote). 4. There is a zero-size buffer
    *      when processing SuccessFetchResult for a shuffle chunk (local or remote).
    */
    ...
    val fallbackBlocksByAddr: Iterator[(BlockManagerId, collection.Seq[(BlockId, Long, Int)])] =
      blockId match {
        case shuffleBlockId: ShuffleMergedBlockId =>
          ...
          mapOutputTracker.getMapSizesForMergeResult(shuffleBlockId.shuffleId, shuffleBlockId.reduceId)
        case _ =>
          ...
          mapOutputTracker.getMapSizesForMergeResult(shuffleChunkId.shuffleId, shuffleChunkId.reduceId, chunkBitmap)
      }
